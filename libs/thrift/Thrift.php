...
abstract class TBase {
  static $allowEnumConversion = true;	// enum I32 is automatically converted to string
  static $convertToArrays = true;	// received objects are converted to arrays
  static $allowOnlyDefinedItems = true;	// in structs, allow only items which exists. Otherwise, throw exception (very usefull for debugging typos)
  ...
  public function __construct($spec=null, $vals=null) {
    if (is_array($spec) && is_array($vals)) {
      ...
      // Check values, if there's some more
      if(self::$allowOnlyDefinedItems) foreach ($vals as $fname => $val) {
        if(!isset($usedFields[$fname])) throw new Exception("Field $fname not exists");
      }
    }
  }

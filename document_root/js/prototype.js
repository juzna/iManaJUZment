...
Ajax.Request = Class.create(Ajax.Base, {
  ...
  request: function(url) {
    var off = url.indexOf(" ");
    if (off >= 0) {
      this.selector = url.slice(off, url.length);
      url = url.slice(0, off);
    }
...
Ajax.Updater = Class.create(Ajax.Request, {
  ...
  updateContent: function(responseText) {
    ...
    if (receiver = $(receiver)) {
      if(this.selector) {
        responseText = new Element('div').update(responseText.stripScripts()).select(this.selector)[0];
      }
...
(function() {
  ...
  function addLiveHandler(name, cb) {
    if(Object.isFunction(name)) {
      cb = name;
      name = 'anonymous-' + onLiveHandlers.length;
    }
  ...
  function onReady(cb) {
    if(document.loaded) cb.defer();
    else Event.observe(document, 'dom:loaded', cb);
  }
  ...
  Event.observe(document, 'ajax:updated', function(ev) { executeLiveHandlers(ev.target); } );
